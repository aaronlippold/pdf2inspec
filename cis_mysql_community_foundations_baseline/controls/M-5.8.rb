...
Ensure only accounts designated for slave users are granted
this privilege.
"
  tag "fix": "Perform the following steps to remediate this setting: Enumerate
the non-slave users found in the result set of
the audit procedure
 For each user, issue the following SQL statement (replace
'<user>' with the non-
slave user):
REVOKE REPLICATION SLAVE ON *.* FROM <user>;
Use the REVOKE statement to remove the SUPER privilege
from users who shouldn't have
it.
"
end
