...
networks. SSL/TLS should be enforced on a per-user basis for users which enter
the system through the network.  The SSL/TLS-protected MySQL protocol helps to
...
Ensure the ssl_type for each user returned is equal to
ANY, X509, or SPECIFIED.
...
Note that REQUIRE SSL only enforces SSL. There are options
like REQUIRE X509, REQUIRE
ISSUER, REQUIRE SUBJECT which can be used to further
restrict connection options.
"
