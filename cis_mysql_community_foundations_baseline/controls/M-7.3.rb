...
https://dev.mysql.com/doc/refman/5.7/en/assigning-passwords.html"
  ...
  tag "cis_level": 1
  tag "audit text": "Execute the following SQL query to determine if any
users have a blank password:
SELECT User,host
FROM mysql.user
WHERE authentication_string='';
No rows will be returned if all accounts have a
password set.
"
  tag "fix": "For each row returned from the audit procedure, set a
password for the given user using
the following statement (as an example):
SET PASSWORD FOR <user>@'<host>' = '<clear password>'
NOTE: Replace <user>, <host>, and <clear password> with
appropriate values.
"
end
