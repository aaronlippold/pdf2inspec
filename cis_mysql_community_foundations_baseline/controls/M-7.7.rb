...
The above query will return zero rows if no anonymous
accounts are present.
"
  tag "fix": "Perform the following actions to remediate this setting:
Enumerate the anonymous users returned from executing the
audit procedure For each anonymous user, DROP or assign them a name
NOTE: As an alternative, you may execute the
mysql_secure_installation utility.
"
  tag "Default Value": "Using the standard installation script,
mysql_install_db, it will\ncreate two anonymous\naccounts: one for the host
'localhost' and the other for the\nnetwork interface's IP address.\n"
end
