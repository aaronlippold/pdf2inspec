...
http://dev.mysql.com/doc/refman/5.7/en/server- options.html#option_mysqld_user"
  tag "severity": "medium"
  tag "cis_id": "1.2"
  tag "cis_control": ["No CIS Control", "6.1"]
  tag "cis_level": 1
  tag "audit text": "Execute the following command at a terminal prompt
to assess this recommendation:
ps -ef | egrep '^mysql.*$'
If no lines are returned, then this is a finding.
NOTE: It is assumed that the MySQL user is mysql.
Additionally, you may consider running
sudo -l as the MySQL user or to check the sudoers
file.
"
  tag "fix": "Create a user which is only used for running MySQL
and directly related processes. This
